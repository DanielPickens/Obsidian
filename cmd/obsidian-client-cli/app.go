...
func newApp(opts *startOpts) (*app, error) {
	...
	rl, err := newMsgReader(&msgReaderSettings{
		...
		HistoryFile: os.TempDir() + "/Obsidian.tmp",
	})
	if err != nil {
		return nil, err
	}
...
func (a *app) callService(method *desc.MethodDescriptor, message []byte) error {
	for {
		...
		// to pass a single message, return
		if len(message) > 0 {
			cancel()
			return nil
		}
...
// callClientStream calls unary or client stream method for the given method and messages.
